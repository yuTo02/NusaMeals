...
type MenuRepository interface {
	...
	CreateMenu(req *request.Menu) error
...
func (r *menuRepository) GetMenusByCategoryName(categoryName string) ([]model.Menu, error) {
	var menus []model.Menu
	err := r.db.Preload("Category").Where("category = ? AND name = ?", categoryName, categoryName).Find(&menus).Error
	if err != nil {
		return nil, err
	}
	return menus, nil
...
func (r *menuRepository) CreateMenu(req *request.Menu) error {
	menu := model.Menu{
		Name:        req.Name,
		Price:       req.Price,
